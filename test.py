...
def extract_complex_features(csi_data):
    # 示例：提取每个子载波的平均幅度和相位
    amplitudes = np.abs(csi_data)
    phases = np.angle(csi_data)
    ifft   = np.fft.ifft(csi_data)
    ifft_amplitudes = np.abs(ifft)
    ifft_phases = np.angle(ifft)

    fig, axs = plt.subplots(4, 1, figsize=(10, 6))

    # 绘制第一个图表
    axs[0].plot(amplitudes[0,0,3])
    axs[0].set_title('First Plot')

    # 绘制第二个图表
    axs[1].plot(phases[0,0,3])
    axs[1].set_title('Second Plot')

    axs[2].plot(ifft_amplitudes[10,0,30])
    axs[2].set_title('Second Plot')

    axs[3].plot(ifft_phases[10,0,30])
    axs[3].set_title('Second Plot')
    plt.show()

    avg_amplitude = np.mean(amplitudes, axis=(1, 2, 3))
    avg_phase = np.mean(phases, axis=(1, 2, 3))
    return np.concatenate([avg_amplitude[:, None], avg_phase[:, None]], axis=1)
